...
Before we can state the existence theorem for the Leray spectral sequence, which will also serve as its definition, we will have to introduce the notion of a hihger direct imgage sheaf. 

\begin{definition}[Higher Direct Image Sheaf \cite{hartshorne}]
Let $\mathcal{F}$ be a sheaf on $X$ and $f: X\to Y$ be a map. Then we define the \textit{$q$-th higher direct image sheaf of $\mathcal{F}$ under the map $f$} to be the sheaf $R^qf_*(\mathcal{F})$ where for every open $U\subset Y$
\begin{equation*}
    (R^qf_*(\mathcal{F}))(U) =H^q(f^{-1}(U);\  \mathcal{F}).
\end{equation*}
    
\end{definition}
Intuitively, the higher direct image sheaf $R^qf_*\mathcal F$ records how the $q$-dimensional cohomological information in the fibers and nearby fibers varies as we move around the base space. With this definition in hand we can now state the theorem.
\begin{theorem}[Leray Spectral Sequence]
    Let $X$ and $Y$ be paracompact  spaces and let $f:X\to Y$ be a continuous map. Let $\mathcal{F}$ be a sheaf of abelian groups on $X$. Then there exists a spectral sequence in which
    \begin{equation*}
        E^{p,q}_2  = H^p\left(Y;\  R^q f_* (\mathcal{F})\right)
    \end{equation*}
    and which converges to 
    \begin{equation*}
        E^{p+q} = H^{p+q}(X;\  \mathcal{F}).
    \end{equation*}
\end{theorem}

\begin{definition}[System of Local Coefficients on $X$]
   Let $X$ be a topological space. Then we use the term \textit{system of local coefficients on $X$} to denote a sheaf on $X$ that is locally constant.

    A local system is called \textit{simple} if the fundamental group of $X$ acts trivially on its fibers.   

   
\end{definition}

If the local system is simple or constant, the Leray spectral sequence specializes to the so-called Leray-Serre spectral sequence which has the following property.

\begin{theorem}[Leray-Serre Spectral Sequence \cite{mccleary}]
    Let $R$ be a commutative ring with unit.
    Suppose $F\hookrightarrow E \overset{\pi}{\to} X$ is a fibration, where $F$ is connected and $X$ is path-connected. Then there exists a first quadrant spectral sequence of algebras, $\{E_r^{*,*}, d_r\}$ which converges to 
    \begin{equation*}
        H^*(E;\ R)
    \end{equation*}
    as an algebra where
    \begin{equation*}
        E^{p,q}_2\cong H^p(X; \ H^q(F, R)),
    \end{equation*}
    the cohomology of the space $X$ with local coefficients in the cohomology of the fiber of $\pi$.
\end{theorem}
...
\begin{definition}[Totally Nonohomologous to Zero]
...
\begin{proof}
    ...
    \end{equation*}
   The fiber $\P^{r-1}$ is famously connected. It is also nonhomologous to zero in $\P[E]$  over $\Z$.
