...
\begin{hwexercise} Prove the fundamental theorem of calculus we used in the de Rham theorem: if $f(x_1, \ldots, x_n)$ is a differentiable function on the unit ball $B \subseteq \R^n$ so that $f(0,0, \ldots, 0) = 0$, then
\begin{align*}
    f(x_1, \ldots, x_n) = \sum_{i=1}^n x_i\int_0^1 \frac{df}{dx_i}(tx_1,tx_2, \ldots, tx_n) dt.
\end{align*}
\end{hwexercise}
